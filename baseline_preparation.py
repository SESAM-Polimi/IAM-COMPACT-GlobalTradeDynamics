...
#%% Parsing database aggregated in db_building.py
...
#%% Read inventories from excel and add supply chains to the database
db.read_add_sectors_excel(master_file_path,read_inventories=True)
db.add_sectors()

#%% Read import shares for commodities
import_shares = pd.read_excel(os.path.join(folder,paths['scenarios']['import_shares']),sheet_name=None,index_col=None)
